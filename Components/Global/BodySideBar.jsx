...
const NFTCard = ({ nft, index }) => (
  <div key={index} className="card-small">
    <div className="author">
      ...
      <div className="info">
        <h6><a>{nft?.name}</a></h6>
        <p><a>{SHORTEN_ADDRESS(nft?.mint)}</a></p>
      </div>
    </div>
    <a href={`/nft-details?mint=${nft?.mint}`} target="_blank" className="date">View</a>
  </div>
);
...
const TopCreator = ({ name, index, title, address }) => (
  <div className={`widget-creators-item flex items-center ${index === 5 ? "" : "mb-20"}`}>
    ...
    <div className="author flex items-center flex-grow">
      <img src={`/assets/images/avatar/avatar-small-0${index}.png`} alt="Creator Avatar" />
      <div className="info">
        <h6><a>{name}</a></h6>
        <span><a>{title}</a></span>
      </div>
    </div>
    <a href={`/seller?address=${address}`} className="tf-button style-2 h50">Profile</a>
  </div>
);

const TrendingCoins = ({ name, index, image }) => (
  <div className={`widget-coins-item flex items-center ${index === 5 ? "" : "mb-20"}`}>
    <img src={`/assets/images/box-icon/${image}`} alt={name} />
    <p><a>{name}</a></p>
  </div>
);

const HistoryCard = ({ name, notify, index }) => (
  <div className={`widget-creators-item flex items-center ${index === 5 ? "" : "mb-20"}`}>
    <div className="author flex items-center flex-grow">
      <img src="/logo-solana.png" style={{ width: "40px", height: "auto" }} alt="Solana Logo" />
      <div className="info">
        <h6><a>{name}</a></h6>
        <span><a>{notify}</a></span>
      </div>
    </div>
...
const BodySideBar = ({ reCall }) => {
  ...
  const [featureNFt, setFeatureNFt] = useState(null);
  ...
  useEffect(() => {
    ...
    if (sellers) setActiveSeller(JSON.parse(sellers));
    if (historys) setOrderHistory(JSON.parse(historys));
    if (userNFTs) {
    ...
    <div className="side-bar">
      {nfts.length > 0 && (
        <>
          <div className="widget widget-recently">
            ...
            <div className="card-small-main">
              <img
                ...
                alt={featureNFt?.name || "Featured NFT"}
              />
              <div className="card-bottom">
                <h5><a href="#">{featureNFt?.name}</a></h5>
                <span className="date">{SHORTEN_ADDRESS(featureNFt?.mint)}</span>
              </div>
            </div>
            {nfts.slice(0, 4).map((nft, index) => <NFTCard key={index} nft={nft} index={index} />)}
          </div>
          ...
          <div className="widget widget-creators">
            ...
            {activeSeller.slice(0, 5).map((item, index) => (
              <TopCreator key={index} name={SHORTEN_ADDRESS(item)} title="Creator" index={index + 1} address={item} />
            ))}
          </div>
        </>
      )}
      ...
      {orderHistory.length > 0 && (
        <div className="widget widget-history">
          ...
          {orderHistory.slice(0, 5).map((item, index) => (
            <HistoryCard key={index} name={SHORTEN_ADDRESS(item?.nft_address)} notify={`Sold at ${item?.price} SOL`} />
          ))}
        </div>
      )}
    </div>
